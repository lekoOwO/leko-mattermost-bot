...
use super::reply_helpers::{empty_json_reply, ephemeral_json_reply, get_form_field, IconConfig};
use crate::AppState;
...
pub async fn handle_sticker_command_impl(
    ...
    let stickers = match sticker_db.search_async(&text, None).await {
        ...
        Err(e) => {
            error!("搜尋貼圖失敗: {}", e);
            return Ok(ephemeral_json_reply("搜尋貼圖失敗，請稍後再試", icon));
        }
    };
    ...
    if stickers.is_empty() {
        ...
        return Ok(ephemeral_json_reply(message, icon));
    }
    ...
    if !response_url.is_empty() {
            ...
            .await
        {
            ...
            return Ok(ephemeral_json_reply("發送貼圖選擇器失敗，請稍後再試", icon));
        }
        info!(
        ...
        Ok(ephemeral_json_reply("無法發送貼圖選擇器", icon))
    }
}
